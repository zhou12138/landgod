...
landgod start --ui --demo --approval-mode auto --config C:\edge_workspace_1\shiproom-config.yaml

# macOS
landgod start --ui --demo --approval-mode auto --config ~/shiproom-config.yaml
```
...
>
> ⚠️ `--demo` mode disables tool security restrictions (command allowlist, content filtering, etc.). It no longer controls approval. Use `--approval-mode auto` for fully unattended demos. **Use only for demos and local testing.**
